<script lang="ts">
  ...
  import { actorNFT } from "$lib/motokoImports/backend";
...
let nftName: string = "Loading..."; // Default text before fetching the actual name

// Fetch NFT name from backend when the component mounts
onMount(async () => {
  try {
    nftName = await actorNFT.icrc7_symbol(); // Fetch NFT name from the backend canister
  } catch (error) {
    console.error("Error fetching NFT name:", error);
    nftName = "Failed to load NFT Name";
  }
});
      ...
      {#key $accountType.value}
        <Select.Root>
          ...
          <Select.Content class="shadow-lg backdrop-blur-xl">
            ...
            <p class="text-sm text-center my-3">Amount: {nftName} </p>
            <hr />
